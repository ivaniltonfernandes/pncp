...
const ApiPNCP = {
  ...
  fetchAllPages: async (baseUrl, params, options = {}) => {
    const {
      ...
      timeoutMs = 20000,
      pageDelayMs = 120,
      maxPages = 80,      // limite de segurança
      maxItems = 15000,   // limite de segurança
      onProgress
    } = options;

    const pageSize = params?.tamanhoPagina ? Number(params.tamanhoPagina) : 500;
    let page = params?.pagina ? Number(params.pagina) : 1;
    ...
    while (true) {
      ...
      const url = ApiPNCP.buildUrl(baseUrl, { ...params, pagina: page, tamanhoPagina: pageSize });
      const json = await ApiPNCP.fetchJsonWithTimeout(url, timeoutMs, signal);
    ...
    return { data: all, meta: { truncated, pagesFetched: page, totalPages } };
  }
};
