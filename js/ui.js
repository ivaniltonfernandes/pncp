...
function openPdfModal(title) {
  ...
  ttl.textContent = title || "Extrair do edital (PDF)";
  body.innerHTML = `
    <div class="flex items-center gap-3 text-slate-700">
      <div class="w-5 h-5 border-2 border-slate-300 border-t-blue-600 rounded-full animate-spin"></div>
      <div id="pdfModalStatus" class="text-sm font-medium">Preparando...</div>
    </div>
    <div class="mt-4 text-xs text-slate-500">
      Observação: se o PDF estiver hospedado fora do PNCP, o navegador pode bloquear a leitura automática (CORS).
    </div>
  `;
...
function setPdfModalStatus(msg) {
  const el = document.getElementById("pdfModalStatus");
  if (el) el.textContent = msg || "";
}

function setPdfModalHtml(html) {
  const body = document.getElementById("pdfModalBody");
...
function extractInfoFromTextByPage(pages) {
  // pages: [{page, text}]
  const results = {
    plantao: null,
    cargaHoraria: null,
  ...
  const choose = (current, candidate) => {
    ...
    // Preferir candidato com palavra-chave contextual
  ...
  pages.forEach(({ page, text }) => {
    ...
    if (!results.plantao && t.includes("plantao")) {
      results.plantao = { value: "Sim", page, snippet: raw.slice(0, 240), score: 1 };
    }

    // Valor (R$...) com contexto
    const moneyMatches = raw.match(moneyRe) || [];
    if (moneyMatches.length) {
      // pega a primeira ocorrência com melhor contexto
      let best = null;
      for (const m of moneyMatches.slice(0, 10)) {
        ...
        const cand = { value: m, page, snippet: ctx.trim(), score: sc };
        best = choose(best, cand);
      }
    ...
    if (!results.cargaHoraria) {
      ...
      if (hourMatch) {
        ...
        const sc = contextScore(ctx, ["carga", "horaria", "plantao", "jornada", "escala", "semana", "mensal"]);
        results.cargaHoraria = { value: hourMatch[0], page, snippet: ctx.trim(), score: sc };
      }
    }

    // Escala (12x36 etc.)
    if (!results.escala) {
      const escalaMatch = raw.match(/\b(\d{1,2})\s*x\s*(\d{1,2})\b/);
      if (escalaMatch) {
        const idx = raw.indexOf(escalaMatch[0]);
        const ctx = raw.slice(Math.max(0, idx - 90), Math.min(raw.length, idx + 160));
        const sc = contextScore(ctx, ["escala", "plantao", "jornada", "revezamento"]);
        results.escala = { value: escalaMatch[0].replace(/\s+/g, ""), page, snippet: ctx.trim(), score: sc };
      }
    }

    // Prazo (data dd/mm/aaaa com contexto)
    const dates = raw.match(dateRe) || [];
    if (dates.length) {
      let best = null;
      for (const d of dates.slice(0, 12)) {
        const idx = raw.indexOf(d);
        const ctx = raw.slice(Math.max(0, idx - 90), Math.min(raw.length, idx + 180));
        const sc = contextScore(ctx, ["proposta", "inscric", "encerr", "prazo", "credenci", "sessao", "abertura"]);
        const cand = { value: d, page, snippet: ctx.trim(), score: sc };
        best = choose(best, cand);
      }
...
async function extractFromEditalPdf(item) {
  try {
    openPdfModal("Extrair do edital (PDF)");
    setPdfModalStatus("Preparando...");

    if (!PdfExtract.ensurePdfJs()) {
      setPdfModalHtml(`
        <div class="text-sm text-red-700 font-semibold">Não foi possível carregar o leitor de PDF.</div>
        <div class="text-xs text-slate-600 mt-2">Verifique sua conexão e tente novamente.</div>
      `);
      return;
    }
    ...
    if (key && PdfExtract.cache.has(key)) {
      setPdfModalHtml(PdfExtract.cache.get(key));
      return;
    }
    ...
    if (!parsed || !parsed.cnpj || !parsed.ano || !parsed.seq) {
      ...
      setPdfModalHtml(`
        ...
        <div class="text-xs text-slate-600 mt-2">Abra no portal e consulte os anexos pelo botão “Ver Oficial”.</div>
    ...
    const getArquivoName = (a) => String(a?.nomeArquivo || a?.nome || a?.titulo || a?.descricao || a?.tipoDocumentoNome || "PDF");
    const isPdf = (a) => {
      const n = getArquivoName(a).toLowerCase();
      const mime = String(a?.contentType || a?.mimeType || "").toLowerCase();
      return n.endsWith(".pdf") || mime.includes("pdf");
    };

    const buildPdfUrl = (a, base, seqUsed) => {
      const seqDoc = ApiPNCP.onlyDigits(a?.sequencialDocumento || a?.sequencial || a?.id || "");
      const direct = a?.documentoUri || a?.uri || a?.url || "";
      if (direct) return normalizeUrl(direct) || direct;
      if (seqDoc) return `${base}/orgaos/${parsed.cnpj}/compras/${parsed.ano}/${seqUsed}/arquivos/${seqDoc}`;
      return "";
    };

    const mergeField = (dst, src, docMeta, fieldKey) => {
      if (dst[fieldKey]) return;
      if (!src || !src[fieldKey]) return;
      dst[fieldKey] = {
        ...src[fieldKey],
        docName: docMeta.docName,
        docUrl: docMeta.docUrl
      };
    };

    const mergeAll = (dst, src, docMeta) => {
      mergeField(dst, src, docMeta, "valor");
      mergeField(dst, src, docMeta, "plantao");
      mergeField(dst, src, docMeta, "cargaHoraria");
      mergeField(dst, src, docMeta, "escala");
      mergeField(dst, src, docMeta, "prazo");
    };

    const allDone = (r) => Boolean(r.valor && r.plantao && r.cargaHoraria && r.escala && r.prazo);


    
     // {docName, ok, reason, url}

    // 1) Tentar extrair primeiro dos ITENS (API)
    setPdfModalStatus("Consultando itens da contratação...");
    try {
      const itensResp = await ApiPNCP.listCompraItens(parsed.cnpj, parsed.ano, parsed.seq, { timeoutMs: 45000 });
      const itens = itensResp?.itens || [];
      if (Array.isArray(itens) && itens.length) {
        const extractedItems = extractInfoFromItems(itens);
        mergeAll(combined, extractedItems, { docName: "Itens (API)", docUrl: "" });
        attempts.push({ docName: "Itens (API)", ok: true, reason: "", url: "" });
      } else {
        attempts.push({ docName: "Itens (API)", ok: false, reason: "Nenhum item retornou pela API.", url: "" });
      }
    } catch (e) {
      const msg = String(e?.message || e || "Falha ao consultar itens.");
      attempts.push({ docName: "Itens (API)", ok: false, reason: msg, url: "" });
    }



    setPdfModalStatus("Consultando anexos do PNCP...");

    const arquivosResp = await ApiPNCP.listCompraArquivos(parsed.cnpj, parsed.ano, parsed.seq, { timeoutMs: 45000 });
    const arquivos = arquivosResp?.arquivos || [];
    const base = arquivosResp?.base || "https://pncp.gov.br/pncp-api/v1";
    const seqUsed = arquivosResp?.seqUsed || parsed.seq;

    if (!arquivos.length) {
      const link = buildSafeLink(item);
      setPdfModalHtml(`
        <div class="text-sm text-slate-800 font-semibold">Nenhum anexo retornou pela API para esta contratação.</div>
        <div class="text-xs text-slate-600 mt-2">Em alguns casos, o edital fica somente no sistema de origem do órgão.</div>
        ${link ? `<div class="mt-3"><a class="text-blue-700 underline text-sm" target="_blank" rel="noopener noreferrer" href="${escapeHtml(link)}">Abrir no portal PNCP</a></div>` : ""}
      `);
      return;
    }

    // ordena anexos por relevância (PDF e termos como “edital”, “termo de referência”, etc.)
    const ranked = (typeof ApiPNCP.rankArquivosParaExtracao === "function")
      ? ApiPNCP.rankArquivosParaExtracao(arquivos)
      : arquivos.slice();

    // mantém apenas PDFs se houver pelo menos um PDF
    const pdfsOnly = ranked.filter(isPdf);
    const candidates = pdfsOnly.length ? pdfsOnly : ranked;

    if (!candidates.length) {
      // Se já conseguimos alguma informação pelos itens (API), seguimos para montar a resposta sem PDFs.
      if (!(combined.valor || combined.plantao || combined.cargaHoraria || combined.escala || combined.prazo)) {
        setPdfModalHtml(`
          <div class="text-sm text-slate-800 font-semibold">Nenhum anexo disponível para leitura.</div>
        `);
        return;
      }
    }

    const maxDocs = Math.min(8, candidates.length);
    
     // {docName, ok, reason, url}

    for (let i = 0; i < maxDocs; i++) {
      const a = candidates[i];
      const docName = getArquivoName(a);
      const docUrl = buildPdfUrl(a, base, seqUsed);

      if (!docUrl) {
        attempts.push({ docName, ok: false, reason: "Sem URL do documento.", url: "" });
        continue;
      }

      setPdfModalStatus(`Analisando anexo ${i + 1}/${maxDocs}: ${docName}`);

      try {
        const bin = await ApiPNCP.fetchBinary(docUrl, { timeoutMs: 75000 });
        const data = bin?.arrayBuffer;
        if (!data) throw new Error("Falha ao baixar o documento.");

        const task = pdfjsLib.getDocument({ data });
        const pdf = await task.promise;

        const totalPages = pdf.numPages || 0;
        const maxPages = Math.min(PdfExtract.maxPages, totalPages || PdfExtract.maxPages);

        const pages = [];
        for (let p = 1; p <= maxPages; p++) {
          setPdfModalStatus(`Analisando anexo ${i + 1}/${maxDocs}: ${docName} (página ${p}/${maxPages})...`);
          const page = await pdf.getPage(p);
          const txt = await page.getTextContent();
          const strings = (txt.items || []).map((it) => it.str).join(" ");
          pages.push({ page: p, text: strings });
        }

        const extracted = extractInfoFromTextByPage(pages);
        mergeAll(combined, extracted, { docName, docUrl });

        attempts.push({ docName, ok: true, reason: "", url: docUrl });

        if (allDone(combined)) break;

      } catch (e) {
        const msg = String(e?.message || e || "Falha ao ler o PDF.");
        attempts.push({ docName, ok: false, reason: msg, url: docUrl });
        // tenta próximo anexo automaticamente
        continue;
      }
    }

    const fmt = (obj) => {
      if (!obj) return `<div class="text-xs text-slate-500">Não localizado.</div>`;
      return `
        <div class="text-sm font-semibold text-slate-900">${escapeHtml(obj.value)}</div>
        <div class="text-[11px] text-slate-500 mt-1">${(obj.page !== undefined && obj.page !== null && String(obj.page).trim() !== "") ? `Página ${escapeHtml(String(obj.page))}` : "Fonte"}${obj.docName ? ` · ${escapeHtml(obj.docName)}` : ""}</div>
        ${obj.docUrl ? `<div class="mt-2"><a class="text-blue-700 underline text-xs" target="_blank" rel="noopener noreferrer" href="${escapeHtml(obj.docUrl)}">Abrir PDF</a></div>` : ""}
        <div class="text-[11px] text-slate-600 mt-2 bg-slate-50 border border-slate-100 rounded-lg p-3">${escapeHtml(obj.snippet || "").slice(0, 650)}</div>
      `;
    };

    const attemptsHtml = attempts.map(a => {
      const badge = a.ok
        ? `<span class="px-2 py-0.5 rounded-full text-[11px] bg-emerald-50 text-emerald-700 border border-emerald-100">Analisado</span>`
        : `<span class="px-2 py-0.5 rounded-full text-[11px] bg-rose-50 text-rose-700 border border-rose-100">Ignorado</span>`;
      const reason = a.ok ? "" : `<div class="text-[11px] text-rose-700 mt-1">${escapeHtml(a.reason).slice(0, 180)}</div>`;
      const link = a.url ? `<a class="text-blue-700 underline text-[11px]" target="_blank" rel="noopener noreferrer" href="${escapeHtml(a.url)}">Abrir</a>` : "";
      return `
        <div class="flex items-start justify-between gap-3 border border-slate-200 rounded-lg p-3">
          <div class="min-w-0">
            <div class="text-sm font-semibold text-slate-800 truncate" title="${escapeHtml(a.docName)}">${escapeHtml(a.docName)}</div>
            ${reason}
          </div>
    ...
    const html = `
      <div class="flex items-start justify-between gap-3">
        <div>
          <div class="text-base font-bold text-slate-900">Extração do edital (PDF)</div>
          <div class="text-xs text-slate-500 mt-1">Tentativas automáticas: ${attempts.length} anexo(s)</div>
        </div>
        <button type="button" class="text-slate-500 hover:text-slate-700 text-sm" onclick="closePdfModal()">✕</button>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
        <div class="bg-white border border-slate-200 rounded-xl p-4">
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Valor / remuneração</div>
          ${fmt(combined.valor)}
        </div>
        <div class="bg-white border border-slate-200 rounded-xl p-4">
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Plantão</div>
          ${fmt(combined.plantao)}
        </div>
        <div class="bg-white border border-slate-200 rounded-xl p-4">
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Carga horária</div>
          ${fmt(combined.cargaHoraria)}
        </div>
        <div class="bg-white border border-slate-200 rounded-xl p-4">
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Escala</div>
          ${fmt(combined.escala)}
        </div>
        <div class="bg-white border border-slate-200 rounded-xl p-4 md:col-span-2">
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Prazo / data no edital</div>
          ${fmt(combined.prazo)}
        </div>
      </div>

      <div class="mt-6">
        <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Fontes analisadas</div>
        <div class="grid grid-cols-1 gap-2">
          ${attemptsHtml || `<div class="text-xs text-slate-500">Nenhum anexo foi analisado.</div>`}
        </div>
      </div>

      <div class="mt-5 text-xs text-slate-500">
        Observações: (1) PDFs fora do domínio do PNCP podem ter bloqueio CORS no navegador. (2) PDFs escaneados podem não ter texto pesquisável.
      </div>
    `;

    if (key) PdfExtract.cache.set(key, html);
    setPdfModalHtml(html);

  } catch (err) {
    const msg = (err && err.message) ? err.message : String(err);
    setPdfModalHtml(`
      <div class="text-sm text-red-700 font-semibold">Falha ao extrair do edital (PDF).</div>
      <div class="text-xs text-slate-600 mt-2">${escapeHtml(msg)}</div>
      <div class="text-xs text-slate-500 mt-3">Se o documento estiver fora do PNCP, pode haver bloqueio de leitura automática.</div>
    `);
  }
}
...
function renderVacancies() {
  ...
  vacancies.forEach(item => {
      ...
      <div class="mt-4 pt-4 border-t border-slate-100 flex items-center justify-between">
        ...
        ${linkSeguro ? `
          <div class="flex items-center gap-2">
            <button type="button" data-action="extract-pdf" class="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold px-3 py-2 rounded-lg transition-all shadow-sm">
              Extrair do edital
            </button>
    ...
    const btnPdf = card.querySelector('[data-action="extract-pdf"]');
    if (btnPdf) {
      btnPdf.addEventListener('click', (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        extractFromEditalPdf(item);
      });
    }
