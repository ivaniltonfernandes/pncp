...
function buildSafeLink(item) {
  ...
  // Tenta extrair ANO/NUMERO do id PNCP quando disponível (mais confiável)
  // Ex.: "11301094000155-1-31/2026"
  const idPncp = String(ApiPNCP.pick(item, ["idContratacaoPncp", "idPncp", "id"]) || "").trim();
  const m = idPncp.match(/(\d{14})-\d+-([0-9]+)\/(\d{4})/);
  const idCnpj = m ? m[1] : "";
  const idNumero = m ? m[2] : "";
  const idAno = m ? m[3] : "";

  const pickAnoNumero = () => {
    const ano = ApiPNCP.pick(item, ["anoCompra", "ano", "anoContratacao", "anoPublicacao"]) || idAno;
    const numero = ApiPNCP.pick(item, ["numeroCompra", "sequencialCompra", "numero", "sequencial", "numeroContratacao"]) || idNumero;
    return { ano: String(ano || "").trim(), numero: String(numero || "").trim() };
  };

  const { ano, numero } = pickAnoNumero();
  const cnpjOk = (cnpj.length === 14 ? cnpj : (idCnpj.length === 14 ? idCnpj : ""));

  // Só monta link oficial se tiver CNPJ + ANO + NUMERO
  if (item.tipoDocumento === "edital" && cnpjOk && ano && numero) {
    linkSeguro = `https://pncp.gov.br/app/editais/${cnpjOk}/${ano}/${numero}`;
  } else if (item.tipoDocumento === "ata" && cnpjOk && ano && numero) {
    linkSeguro = `https://pncp.gov.br/app/atas/${cnpjOk}/${ano}/${numero}`;
  } else if (item.tipoDocumento === "contrato" && cnpjOk && ano && numero) {
    linkSeguro = `https://pncp.gov.br/app/contratos/${cnpjOk}/${ano}/${numero}`;
  }

  // Fallback: usa os links retornados pela API se existirem
  if (!linkSeguro) {
    const linkBruto = ApiPNCP.pick(item, ["linkSistemaOrigem", "link", "url"]);
    if (linkBruto && /^https?:\/\//i.test(String(linkBruto))) linkSeguro = String(linkBruto);
  }
