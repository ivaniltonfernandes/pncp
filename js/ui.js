...
const DOCTOR_WORD_RE = /medic[oa]s?/;
const CRM_RE = /crm/;

// Sinais fortes de que o objeto fala de MÉDICOS (evita pegar "clínico" de laboratório etc.)
const DOCTOR_CONTEXT_TERMS = [
  "corpo clinico",
  "equipe medica",
  "medico plantonista",
  "medico da familia",
...
// Stems (texto normalizado, sem acento) para especialidades médicas (match parcial)
const SPECIALTY_STEMS = [
  "pediatr", "psiquiatr", "anestesiolog", "ginecolog", "obstetr",
  "ortoped", "cardiolog", "urolog", "dermatolog", "infectolog",
  "intensiv", "urgenc", "emergenc", "neurolog", "endocrin", "nefrolog",
  "oftalmolog", "otorrin", "radiolog", "cirurg", "oncolog", "reumatolog",
  "gastroenterolog", "pneumolog", "geriat", "clinico geral"
];

// Termos que caracterizam OPORTUNIDADE (contratação/credenciamento/plantões etc.)
...
// Termos "fracos" (só aceitos se houver também algum sinal forte/contexto)
const WEAK_HIRING_TERMS = [
  "contratacao",
  "contratar",
...
// Excluir compras/serviços que NÃO são vaga de médico
const EXCLUDE_TERMS = [
  // compras de itens/insumos
  "medicamento", "medicamentos", "remedio", "farmacia", "farmaceutic",
  "material medico", "materiais medicos", "material medico-hospitalar", "material hospitalar",
  "insumo", "insumos", "equipamento", "equipamentos", "aparelho", "aparelhos",
  "suprimento", "suprimentos", "peca", "pecas", "kit", "luva", "seringa", "agulha",
  "cateter", "curativo", "gaze", "soro", "ampola", "epi", "mascara", "respirador",
  "reagente", "laboratorio", "exame", "exames", "tomografia", "ultrassom", "raio x", "radiologia",
  "oxigenio", "cilindro",

  // SRP/ata (normalmente compras)
  "registro de precos", "ata de registro", "srp",

  // planos/benefícios (não é contratação de médico)
  "plano de saude", "plano odontologico", "assistencia medico-hospitalar", "assistencia medico hospitalar",
  "assistencia medica", "saude suplementar", "seguro saude", "operadora", "beneficiario",
  "coparticipacao", "rede credenciada", "ambulatorial", "hospitalar e ambulatorial",

  // outras áreas que confundem
  "odontolog", "odontologico", "veterinar"
];
...
function buildVacancyText(item) {
  // Junta campos comuns onde o PNCP descreve o objeto, para melhorar o filtro
  const parts = [];

  const obj = ApiPNCP.pick(item, [
    "objetoCompra", "objeto", "descricaoObjeto", "objetoAta", "objetoContrato", "objetoContratacao",
    "descricao", "informacaoComplementar", "justificativa", "observacao"
  ]);
  if (obj) parts.push(String(obj));

  const org = item && (item.orgaoEntidade || item.orgao || {});
  const orgName = ApiPNCP.pick(org, ["razaoSocial", "nomeFantasia", "nome"]);
  if (orgName) parts.push(String(orgName));

  const unidade = item && (item.unidadeOrgao || item.unidade || {});
  const unidadeNome = ApiPNCP.pick(unidade, ["nome"]);
  if (unidadeNome) parts.push(String(unidadeNome));

  const mun = ApiPNCP.pick(item, ["municipioNome", "municipio", "nomeMunicipio"]);
  if (mun) parts.push(String(mun));
...
function scoreDoctorVacancy(text) {
  ...
  // Exclusões fortes (não é vaga de médico)
  if (EXCLUDE_TERMS.some(term => t.includes(term))) {
    return { ok: false, score: -99 };
  }

  // Sinais de médico (evita "medicamento" etc.)
  const hasDoctor =
    DOCTOR_WORD_RE.test(t) ||
    CRM_RE.test(t) ||
    DOCTOR_CONTEXT_TERMS.some(term => t.includes(term)) ||
    SPECIALTY_STEMS.some(st => t.includes(st));

  if (!hasDoctor) return { ok: false, score: 0 };

  // Sinais de oportunidade/contratação
  const hasStrongHiring =
    STRONG_HIRING_TERMS.some(term => t.includes(term)) ||
    /\bcredenciament\w*\b/.test(t) ||
    /\bchamament\w*\b/.test(t) ||
    /\bprocesso\s+seletiv\w*\b/.test(t);

  const hasWeakHiring =
    WEAK_HIRING_TERMS.some(term => t.includes(term)) ||
    /\bcontrat\w*\b/.test(t);

  const okHiring =
    hasStrongHiring ||
    (hasWeakHiring && (t.includes("profission") || t.includes("pessoa fisica") || t.includes("plantao") || t.includes("cooperativa")));

  if (!okHiring) return { ok: false, score: 0 };

  // Regras anti-"compra de insumo"
  if (t.includes("aquisicao") && !t.includes("servic")) return { ok: false, score: -10 };
  if (t.includes("fornecimento") && !t.includes("mao de obra") && !t.includes("profission") && !t.includes("servic")) return { ok: false, score: -10 };

  let score = 0;

  if (DOCTOR_WORD_RE.test(t)) score += 5;
  if (CRM_RE.test(t)) score += 4;
  if (SPECIALTY_STEMS.some(st => t.includes(st))) score += 2;
  if (DOCTOR_CONTEXT_TERMS.some(term => t.includes(term))) score += 2;

  if (t.includes("credenciamento")) score += 5;
  if (t.includes("chamamento publico")) score += 5;
  if (t.includes("processo seletivo") || t.includes("selecionamento")) score += 4;

  if (t.includes("prestacao de servicos medicos") || t.includes("prestacao de servico medico")) score += 4;
  if (t.includes("servicos medicos") || t.includes("servico medico")) score += 4;

  if (t.includes("plantao") || t.includes("plantoes") || t.includes("plantonista") || t.includes("escala de plantao")) score += 3;
  if (t.includes("pessoa fisica") || /\bpf\b/.test(t)) score += 2;
  if (t.includes("cooperativa medica")) score += 2;
  if (t.includes("mao de obra")) score += 1;

  if (t.includes("vaga") || t.includes("vagas")) score += 1;

  // Para ser considerado OK, exigimos pontuação mínima
  const ok = score >= 8;
  return { ok, score };
}
...
// ====== Links oficiais PNCP (Editais/Atas/Contratos) ======

function parsePncpId(raw) {
  const s = String(raw || "").trim();
  if (!s) return null;

  // Formato comum: CNPJ-<algo>-<numero>/<ano>
  let m = s.match(/(\d{14})-[^-]+-([0-9]+)\/(\d{4})/);
  if (m) {
    return {
      cnpj: m[1],
      numero: String(m[2]).replace(/^0+(?=\d)/, ""), // remove zeros à esquerda (mantém '0' se for só zero)
      ano: m[3]
    };
  }

  // Fallback: tenta achar "CNPJ ... numero/ano" em qualquer lugar
  m = s.match(/(\d{14}).*?([0-9]+)\/(\d{4})/);
  if (m) {
    return {
      cnpj: m[1],
      numero: String(m[2]).replace(/^0+(?=\d)/, ""),
      ano: m[3]
    };
  }
...
function normalizePncpNumero(raw) {
  const s = String(raw || "").trim();
  if (!s) return "";
  ...
  // tenta extrair "...-<numero>/<ano>"
  let m = s.match(/-([0-9]+)\/(\d{4})$/);
  if (m) return String(m[1]).replace(/^0+(?=\d)/, "");
...
function normalizePncpAno(raw) {
  const s = String(raw || "").trim();
  if (!s) return "";
  if (/^\d{4}$/.test(s)) return s;

  // tenta extrair ano de ".../<ano>"
  const m = s.match(/\/(\d{4})$/);
  if (m) return m[1];
...
function isValidPncpPortalLink(url, tipoDocumento) {
  const u = String(url || "").trim();
  if (!u) return false;

  if (tipoDocumento === "edital") return /^https:\/\/pncp\.gov\.br\/app\/editais\/\d{14}\/\d{4}\/\d+/.test(u);
  if (tipoDocumento === "ata") return /^https:\/\/pncp\.gov\.br\/app\/atas\/\d{14}\/\d{4}\/\d+/.test(u);
  if (tipoDocumento === "contrato") return /^https:\/\/pncp\.gov\.br\/app\/contratos\/\d{14}\/\d{4}\/\d+/.test(u);
  return /^https?:\/\//.test(u);
}

function canonicalizeUrlMaybe(raw) {
  if (!raw) return "";
  const s = String(raw).trim();
  if (!s) return "";
...
function buildPncpPortalLink(item) {
  const tipo = item?.tipoDocumento || "";

  // 1) se o cache já trouxe um link oficial pronto, use-o
  const fromCache = ApiPNCP.pick(item, ["linkOficial", "linkOficialPncp", "urlOficial"]);
  if (fromCache) {
    const u = canonicalizeUrlMaybe(fromCache);
    if (isValidPncpPortalLink(u, tipo)) return u;
  }

  // 2) cnpj / ano / numero: tenta em várias fontes
  let cnpjRaw = ApiPNCP.pick(item, ["cnpj", "numeroInscricaoCnpj", "cnpjOrgao"]) || (item.orgaoEntidade && item.orgaoEntidade.cnpj) || "";
  let cnpj = ApiPNCP.onlyDigits(cnpjRaw);

  let ano = normalizePncpAno(ApiPNCP.pick(item, ["anoCompra", "anoAta", "anoContrato", "ano"]));
  let numero = "";

  if (tipo === "edital") numero = normalizePncpNumero(ApiPNCP.pick(item, ["numeroCompra", "sequencialCompra", "numero"]));
  else if (tipo === "ata") numero = normalizePncpNumero(ApiPNCP.pick(item, ["numeroAta", "numero"]));
  else if (tipo === "contrato") numero = normalizePncpNumero(ApiPNCP.pick(item, ["numeroContrato", "numero"]));

  // 3) se ainda faltar algo, tenta extrair do id PNCP (quando vier)
  if (!cnpj || !ano || !numero) {
    const idRaw = ApiPNCP.pick(item, [
      "idContratacaoPncp", "idContratacaoPNCP", "idCompraPncp", "idCompraPNCP",
      "idContratacao", "idCompra", "id"
    ]);
    const parsed = parsePncpId(idRaw);
    if (parsed) {
      if (!cnpj) cnpj = parsed.cnpj;
      if (!ano) ano = parsed.ano;
      if (!numero) numero = parsed.numero;
    }
  }

  // 4) monta link oficial do portal
  if (cnpj.length === 14 && /^\d{4}$/.test(ano) && /^\d+$/.test(numero)) {
    if (tipo === "edital") return `https://pncp.gov.br/app/editais/${cnpj}/${ano}/${numero}`;
    if (tipo === "ata") return `https://pncp.gov.br/app/atas/${cnpj}/${ano}/${numero}`;
    if (tipo === "contrato") return `https://pncp.gov.br/app/contratos/${cnpj}/${ano}/${numero}`;
  }

  // 5) fallback: link do sistema origem / url crua
  const linkBruto = ApiPNCP.pick(item, ["linkSistemaOrigem", "link", "url"]);
  if (linkBruto) {
    const u = canonicalizeUrlMaybe(linkBruto);
    if (u) return u;
  }
...
async function updateNationalCardUI() {
  ...
  try {
    ...
    items.forEach(item => {
      ...
      const vacancyText = buildVacancyText(item);
      const scored = scoreDoctorVacancy(vacancyText);
      if (!scored.ok) return;
...
function renderVacancies() {
  ...
  vacancies.forEach(item => {
    ...
    const linkSeguro = buildPncpPortalLink(item);
