...
// Referência: Manual PNCP API Consultas (tabela de domínio: Modalidade de Contratação)
const MODALIDADES_BUSCA = ["6", "8", "2", "3", "7"]; // Pregão Eletrônico, Dispensa, Diálogo, Concurso, Pregão Presencial

// Palavras-chave para detectar documentos com possível vaga/serviço médico
const MEDICAL_KEYWORDS = [
  "médico", "medico", "medicina", "plantão", "plantao",
  "clínico", "clinico", "psiquiatra", "pediatra",
  "saúde", "hospitalar"
];
...
// Tamanhos de página: alguns endpoints podem rejeitar 'tamanhoPagina'.
// O ApiPNCP.fetchAllPages tem fallback automático removendo tamanhoPagina caso dê 400.
const PAGE_SIZES = {
  contratacoes: 100,
  atas: 500,
  contratos: 500
...
async function openState(stateName, stateSigla) {
  ...
  document.getElementById('citiesSubtitle').textContent = `Buscando Editais, Atas e Contratos médicos (últimos ${LOOKBACK_DAYS} dias)...`;
  document.getElementById('citiesGrid').innerHTML = '';
  document.getElementById('loadingCities').classList.remove('hidden');
  showView('cities');

  const { dataInicial, dataFinal } = ApiPNCP.getDateRange(LOOKBACK_DAYS);
  ...
  try {
    ...
    // 1) CONTRATAÇÕES / EDITAIS (filtra por UF via parâmetro 'uf')
      ...
      try {
        const res = await ApiPNCP.fetchAllPages(
          API_EDITAIS,
          {
            ...
            uf: stateSigla,
            pagina: 1,
            tamanhoPagina: PAGE_SIZES.contratacoes
          },
          {
        ...
        warnings.push(`Contratações (mod ${mod}): ${shortErrMessage(err)}`);
      }
    ...
    try {
      const resAtas = await ApiPNCP.fetchAllPages(
        API_ATAS,
        { dataInicial, dataFinal, pagina: 1, tamanhoPagina: PAGE_SIZES.atas },
        {
          signal: currentAbortCtrl.signal,
          timeoutMs: SAFETY_LIMITS.timeoutMs,
          pageDelayMs: SAFETY_LIMITS.pageDelayMs,
      ...
      warnings.push(`Atas: ${shortErrMessage(err)}`);
    }
    ...
    try {
      const resContratos = await ApiPNCP.fetchAllPages(
        API_CONTRATOS,
        { dataInicial, dataFinal, pagina: 1, tamanhoPagina: PAGE_SIZES.contratos },
        {
          signal: currentAbortCtrl.signal,
          timeoutMs: SAFETY_LIMITS.timeoutMs,
          pageDelayMs: SAFETY_LIMITS.pageDelayMs,
      ...
      warnings.push(`Contratos: ${shortErrMessage(err)}`);
    }
    ...
    rawItems.forEach(item => {
      // Filtra por UF quando necessário (contratações já vieram filtradas por uf)
    ...
    const partialMsg = warnings.length ? ` (parcial: ${warnings.length} fonte(s) falharam)` : "";

    if (cityNames.length === 0) {
      document.getElementById('citiesSubtitle').textContent = `Nenhum documento médico encontrado em ${stateSigla} nos últimos ${LOOKBACK_DAYS} dias.${partialMsg}`;

      const details = warnings.length
        ? `<div class="mt-3 text-xs text-slate-500">Dica: algumas fontes falharam. Tente novamente ou reduza o período de busca.<br><span class="text-slate-400">${warnings.map(w => escapeHtmlAttr(w)).join('<br>')}</span></div>`
        : "";

      document.getElementById('citiesGrid').innerHTML =
        `<div class="col-span-full p-8 text-center text-slate-500 bg-white rounded-2xl border border-slate-200">
          Nenhum documento médico encontrado neste estado no período consultado.${details}
        </div>`;
      return;
    }

    document.getElementById('citiesSubtitle').textContent = `${medicalCount} documentos encontrados em ${cityNames.length} municípios${truncMsg}${partialMsg}.`;
    ...
    document.getElementById('citiesSubtitle').innerHTML =
      `<span class="text-red-500 font-medium">Erro ao buscar os dados na API: ${escapeHtmlAttr(shortErrMessage(error))}</span>`;
